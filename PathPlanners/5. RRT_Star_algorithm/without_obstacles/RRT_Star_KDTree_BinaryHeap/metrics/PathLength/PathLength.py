...
for trial in range(num_trials):
    ...
    for i in range(1, len(rrt_star_path)):
        x1, y1 = rrt_star_path[i-1]
        x2, y2 = rrt_star_path[i]
        path_length += math.dist((x1, y1), (x2, y2))

    total_path_length += path_length
    print(f"Trial {trial + 1} - Path Length: {path_length}")

average_path_length = total_path_length / num_trials
print(f"Average Path Length over {num_trials} trials: {average_path_length}")
