...
class Graph:
    ...
    def get_neighbors(self, node):
        ...
        for neighbor in neighbors:
            nx, ny = neighbor
            if 0 <= nx - self.start_x < self.width and 0 <= ny - self.start_y < self.height:
    ...
    def is_obstacle(self, node):
        ...
        for obstacle in obstacles:
            if len(obstacle) == 3:
                ...
                s = (y1 * x3 - x1 * y3 + (y3 - y1) * x + (x1 - x3) * y) * sign
                t = (x1 * y2 - y1 * x2 + (y1 - y2) * x + (x2 - x1) * y) * sign
