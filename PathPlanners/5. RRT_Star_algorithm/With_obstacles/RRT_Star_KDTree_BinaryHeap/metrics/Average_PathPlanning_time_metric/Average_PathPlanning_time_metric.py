...
def run_rrt_star(start_coords, goal_coords):
    rrt = RRTStar(start_coords, goal_coords, step_size=5, max_iter=5000, radius=20)
    path = rrt.rrt_star()
    return path
...
path = run_rrt_star(start_coords, goal_coords)
print(f"Path: {path}")
...
for i in range(num_trials):
    ...
    run_rrt_star(start_coords, goal_coords)
    end_time = time.time()
